...
def main() -> None:
    if not BIN_FILE.exists():
        raise SystemExit(f"No existe el binario: {BIN_FILE}")
    ...
    if broken:
        print("Aviso: ldd no pudo resolver estas librerías:")
        for item in broken:
            print(f"\t{item}")

    if unowned:
        print("Aviso: estas librerías no pertenecen a ningún paquete instalado:")
        for item in unowned:
            print(f"\t{item}")
